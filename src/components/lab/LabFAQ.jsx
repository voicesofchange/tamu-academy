  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
      {FAQS.map((faq, i) => {
        const isOpen = open === i;
        return (
          <div key={i} style={{ border: '1px solid rgba(232,184,91,0.13)', borderRadius: '4px', overflow: 'hidden', backgroundColor: 'rgba(243,234,216,0.015)' }}>
            <button
            ...
            >
              <h3 className="font-heading" style={{ color: '#f8f0df', fontSize: '1rem', fontWeight: 400, margin: 0, lineHeight: 1.4 }}>{faq.q}</h3>
              <span aria-hidden="true" style={{ color: '#e8b85b', fontSize: '1rem', flexShrink: 0, transition: 'transform 0.25s ease', transform: isOpen ? 'rotate(45deg)' : 'none' }}>+</span>
            </button>
            <AnimatePresence initial={false}>
              {isOpen && (
                ...
                >
                  <div style={{ padding: '0 1.5rem 1.25rem', borderTop: '1px solid rgba(232,184,91,0.08)' }}>
                    <p className="font-body" style={{ color: 'rgba(243,234,216,0.72)', fontSize: '0.9rem', lineHeight: 1.8, fontWeight: 300, margin: '1rem 0 0' }}>{faq.a}</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>

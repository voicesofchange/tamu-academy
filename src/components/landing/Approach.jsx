      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          {MODEL_STEPS.map((step, i) => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                <span
                  aria-hidden="true"
                  style={{ width: '28px', height: '28px', borderRadius: '50%', border: '1px solid rgba(232,184,91,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                >
                  <span className="font-body" style={{ color: '#e8b85b', fontSize: '0.65rem', fontWeight: 500 }}>{i + 1}</span>
                </span>
                <h3 className="font-heading" style={{ color: '#f8f0df', fontSize: '1.2rem', fontWeight: 400, margin: 0 }}>{step.label}</h3>
              </div>
              <p className="font-body" style={{ color: 'rgba(243,234,216,0.68)', fontSize: '0.88rem', lineHeight: 1.75, fontWeight: 300, margin: 0, paddingLeft: '2.65rem' }}>
                {step.desc}
              </p>
            </motion.div>
          ))}
        ...
        >
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.55rem' }}>
            {METHODS.map((m) => (
              <span key={m} className="font-body" style={{ color: 'rgba(243,234,216,0.68)', fontSize: '0.83rem', border: '1px solid rgba(232,184,91,0.18)', borderRadius: '2px', padding: '0.3rem 0.8rem', fontWeight: 300 }}>{m}</span>
            ))}
          </div>
        </motion.div>
      </div>

...
const SELF_ATTESTED_KEYS = new Set(['core-media-reviewed', 'amplify-lab']);
...
export default function MhModule6Progress({ courseSlug, moduleRoute, completionRequirements, progressTracking, refreshTrigger = 0 }) {
  ...
  const allFiveComplete =
    progress && Array.isArray(progress.completionKeys) && Array.isArray(progress.completedKeys) &&
  ...
  async function handleMarkComplete(key) {
    ...
    setSavingKey(key); setStatusMessage(null);
    try {
      const res = await base44.functions.invoke('updateMentalHealthProgress', {
        courseSlug, moduleRoute, action: 'acknowledge_module6_requirement', requirementKey: key,
      });
      const data = res && res.data ? res.data : null;
  ...
  async function handleCompleteModule() {
    ...
    if (!allFiveComplete || progress.moduleCompleted) return;
    setCompletionPending(true); setStatusMessage(null);
    try {
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ marginTop: '1.5rem' }}>
            <button type="button" disabled={!allFiveComplete || completionPending || progress.moduleCompleted} onClick={handleCompleteModule} className="font-body"
              style={{ ...completeButtonBase, color: allFiveComplete && !completionPending && !progress.moduleCompleted ? '#1A130E' : 'rgba(245,239,224,0.4)', backgroundColor: allFiveComplete && !completionPending && !progress.moduleCompleted ? '#D4A12A' : 'rgba(212,161,42,0.15)', cursor: allFiveComplete && !completionPending && !progress.moduleCompleted ? 'pointer' : 'not-allowed' }}>

...
export default function MhModule7Progress({ courseSlug, moduleRoute, completionRequirements, progressTracking, refreshTrigger = 0 }) {
  ...
  async function handleMarkComplete(key) {
    ...
    try {
      const res = await base44.functions.invoke('updateMentalHealthProgress', {
        courseSlug, moduleRoute, action: 'acknowledge_module7_requirement', requirementKey: key,
      });
      const data = res && res.data ? res.data : null;
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ marginTop: '1.5rem' }}>
            <button type="button" disabled={!allComplete || completionPending || progress.moduleCompleted} onClick={handleCompleteModule} className="font-body"
              style={{ ...completeButtonBase, color: allComplete && !completionPending && !progress.moduleCompleted ? '#1A130E' : 'rgba(245,239,224,0.4)', backgroundColor: allComplete && !completionPending && !progress.moduleCompleted ? '#D4A12A' : 'rgba(212,161,42,0.15)', cursor: allComplete && !completionPending && !progress.moduleCompleted ? 'pointer' : 'not-allowed' }}>

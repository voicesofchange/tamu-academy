...
export default function MhModule3Progress({
  courseSlug,
  moduleRoute,
  ...
  async function handleMarkComplete(key) {
    ...
    try {
      const res = await base44.functions.invoke('updateMentalHealthProgress', {
        courseSlug,
        moduleRoute,
        action: 'acknowledge_module3_requirement',
        requirementKey: key,
      });
      const data = res && res.data ? res.data : null;
  ...
  async function handleCompleteModule() {
    ...
    if (!allSevenComplete || progress.moduleCompleted) return;
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ marginTop: '1.5rem' }}>
            <button
              type="button"
              disabled={!allSevenComplete || completionPending || progress.moduleCompleted}

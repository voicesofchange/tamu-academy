...
/**
 * MhModule2Progress — Stage 12 progress tracking UI for Module 2.
 *
 * Integrates into the existing completion-requirements section of
 * MhModule2Lesson. Reads all learner-facing wording from the
 * progressTracking prop (sourced from lesson.progressTracking via
 * the authenticated backend response). No progress wording is
 * hardcoded in this file.
 *
 * BEHAVIOR:
 *   - On mount, calls getMentalHealthProgress to load the current
 *     user's Module 2 completion status. Creates no record.
 *   - When eligibleToSave is true (published + enrolled non-admin):
 *     shows the six requirements with completion status, mark-complete
 *     controls for the four self-attestable requirements, and a final
 *     module completion button disabled until all six are complete.
 *   - When eligibleToSave is false (current admin preview): shows
 *     only the unavailableMessage. No mutation is called. No
 *     ModuleProgress record is created.
 *   - The two server-verified keys (knowledge-check-completed,
 *     knowledge-check-passed) are updated only from the
 *     checkMentalHealthKnowledgeCheck server response, triggered
 *     via the refreshTrigger prop after grading.
 *
 * PRIVACY:
 *   - No lab writing, reflection writing, scenario selections, or
 *     knowledge check selections are sent to the backend.
 *   - No local storage, session storage, cookies, or analytics.
 *   - No direct entity access — all reads and writes go through
 *     backend functions.
 */
export default function MhModule2Progress({
  courseSlug,
  moduleRoute,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await base44.functions.invoke('getMentalHealthProgress', {
          courseSlug,
          moduleRoute,
        });
        if (cancelled) return;
  ...
  async function handleMarkComplete(key) {
    ...
    try {
      const res = await base44.functions.invoke('updateMentalHealthProgress', {
        courseSlug,
        moduleRoute,
        action: 'acknowledge_module2_requirement',
        requirementKey: key,
      });
      const data = res && res.data ? res.data : null;
  ...
  async function handleCompleteModule() {
    ...
    try {
      ...
      if (data && data.completed) {
        setProgress((prev) =>
          prev
            ? { ...prev, moduleCompleted: true, completedAt: data.completedAt }
            : prev,
        );
      ...
      {loading ? (
        ...
        <>
              ...
              return (
                ...
                >
                  ...
                  {isSelfAttested && !isCompleted && (
                    ...
                    >
                      {savingKey === key
                        ? progressTracking.savingLabel
                        : progressTracking.markCompleteLabel}
                    </button>
                  )}
                  {isSelfAttested && isCompleted && (
                    <span
          ...
          <div style={{ marginTop: '1.5rem' }}>
            <button
              type="button"
              disabled={!allSixComplete || completionPending || progress.moduleCompleted}
              onClick={handleCompleteModule}
              className="font-body"
              style={{
                ...completeButtonBase,
                color:
                  allSixComplete && !completionPending && !progress.moduleCompleted
                    ? '#1A130E'
            ...
            >
              {completionPending
                ? progressTracking.savingLabel
                : progressTracking.completeModuleLabel}
            </button>
          </div>
          ...
          {progress.moduleCompleted && !statusMessage && (
            ...
            >
              {progressTracking.completedMessage}
            </p>
          )}
        </>
      ) : (
        ...
        >
          ...
          >
            {progressTracking.unavailableMessage}
          </p>
        </div>
      )}
    </div>

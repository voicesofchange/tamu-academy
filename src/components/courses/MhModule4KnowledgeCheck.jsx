...
export default function MhModule4KnowledgeCheck({ courseSlug, moduleSlug, quiz, onGraded }) {
  ...
  return (
    <div>
      {quiz.subtitle && (
        <p className="font-body" style={{ ...bodyText, fontStyle: 'italic', color: 'rgba(232,184,91,0.78)', marginBottom: '1.1rem', fontSize: '0.9rem' }}>{quiz.subtitle}</p>
      )}
            ...
            <section key={q.id} style={questionCardStyle} aria-label={`Question ${qIdx + 1}`}>
                    ...
                    return (
                      <li key={i} style={{ marginBottom: '0.65rem' }}>
                        <label style={style}>
                          ...
                          <span style={{ flex: 1 }}>
                            <strong style={{ color: 'rgba(232,184,91,0.85)', fontWeight: 500 }}>{optionLabel(i)}.</strong> {opt}
                          </span>
                        </label>
                      </li>
                    );
              ...
              {qResult && (
                <div role="status" style={{ marginTop: '0.85rem', padding: '0.85rem 1rem', border: `1px solid ${qResult.isCorrect ? 'rgba(232,184,91,0.4)' : 'rgba(243,234,216,0.25)'}`, borderRadius: '3px', backgroundColor: 'rgba(243,234,216,0.02)' }}>
                  <p className="font-body" style={{ ...bodyText, margin: 0, marginBottom: '0.4rem', color: qResult.isCorrect ? '#e8b85b' : '#e8955c', fontWeight: 500 }}>
                    {qResult.isCorrect ? 'Correct' : 'Reconsider'}
                  </p>
                  <p className="font-body" style={{ ...bodyText, fontStyle: 'italic', margin: 0, color: 'rgba(243,234,216,0.7)', fontSize: '0.88rem' }}>
                    {qResult.feedback}
                  </p>
                </div>
              )}
      ...
      {result && (
        <div role="status" aria-live="polite" style={{ marginTop: '1.5rem', padding: '1.2rem 1.4rem', border: `1px solid ${result.passed ? 'rgba(232,184,91,0.5)' : 'rgba(243,234,216,0.3)'}`, borderRadius: '4px', backgroundColor: 'rgba(243,234,216,0.02)' }}>
          <p className="font-body" style={{ ...bodyText, margin: 0, marginBottom: '0.5rem', color: '#f8f0df', fontWeight: 500, fontSize: '1.1rem' }}>
          ...
          <p className="font-body" style={{ ...bodyText, margin: 0, color: result.passed ? '#e8b85b' : '#e8955c', fontStyle: 'italic' }}>

...
export default function MhModule4KnowledgeCheck({ courseSlug, moduleSlug, quiz, onGraded }) {
  ...
  return (
    <div>
      {quiz.subtitle && (
        <p className="font-body" style={{ ...bodyText, fontStyle: 'italic', color: 'rgba(212,161,42,0.78)', marginBottom: '1.1rem', fontSize: '0.9rem' }}>{quiz.subtitle}</p>
      )}
      ...
      <form onSubmit={handleSubmit} aria-label="Module 4 knowledge check">
            ...
            <section key={q.id} style={questionCardStyle} aria-label={`Question ${qIdx + 1}`}>
                      ...
                      <li key={i} style={{ marginBottom: '0.65rem' }}>
                        <label style={style}>
                          <input type="radio" name={q.id} value={i} checked={isSelected || false} disabled={locked}
                            onChange={() => handleSelect(q.id, i)} style={{ marginTop: '0.2rem' }}
                            aria-label={`Option ${optionLabel(i)}: ${opt}`} />
                          <span style={{ flex: 1 }}>
                            <strong style={{ color: 'rgba(212,161,42,0.85)', fontWeight: 500 }}>{optionLabel(i)}.</strong> {opt}
                          </span>
                        </label>
                      </li>
                    );
              ...
              {qResult && (
                <div role="status" style={{ marginTop: '0.85rem', padding: '0.85rem 1rem', border: `1px solid ${qResult.isCorrect ? 'rgba(212,161,42,0.4)' : 'rgba(245,239,224,0.25)'}`, borderRadius: '3px', backgroundColor: 'rgba(245,239,224,0.02)' }}>
                  <p className="font-body" style={{ ...bodyText, margin: 0, marginBottom: '0.4rem', color: qResult.isCorrect ? '#D4A12A' : '#e8955c', fontWeight: 500 }}>
                    {qResult.isCorrect ? 'Correct' : 'Reconsider'}
                  </p>
                  <p className="font-body" style={{ ...bodyText, fontStyle: 'italic', margin: 0, color: 'rgba(245,239,224,0.7)', fontSize: '0.88rem' }}>
                    {qResult.feedback}
                  </p>
                </div>
              )}
        ...
        {!result && (
          <button type="submit" disabled={pending} style={{ ...submitButtonStyle, cursor: pending ? 'wait' : 'pointer', opacity: pending ? 0.7 : 1 }}>
            {pending ? 'Grading…' : 'Submit answers'}
          </button>
        )}
        {result && (
          <button type="button" onClick={handleRetry} style={retryButtonStyle}>Try again</button>
        )}
        {validationError && (
          <p className="font-body" role="alert" style={{ color: '#e8955c', marginTop: '1rem', marginBottom: 0, fontSize: '0.88rem' }}>{validationError}</p>
        )}
        {serverError && (
          <p className="font-body" role="alert" style={{ color: '#e8955c', marginTop: '1rem', marginBottom: 0, fontSize: '0.88rem' }}>{serverError}</p>
        )}
      </form>
      {result && (
        <div role="status" aria-live="polite" style={{ marginTop: '1.5rem', padding: '1.2rem 1.4rem', border: `1px solid ${result.passed ? 'rgba(212,161,42,0.5)' : 'rgba(245,239,224,0.3)'}`, borderRadius: '4px', backgroundColor: 'rgba(245,239,224,0.02)' }}>
          <p className="font-body" style={{ ...bodyText, margin: 0, marginBottom: '0.5rem', color: '#F5EFE0', fontWeight: 500, fontSize: '1.1rem' }}>
            You answered {result.score} of {result.totalQuestions} questions correctly. Answer at least four of the five questions correctly to pass.
          </p>
          <p className="font-body" style={{ ...bodyText, margin: 0, color: result.passed ? '#D4A12A' : '#e8955c', fontStyle: 'italic' }}>
            {result.passed ? 'You passed this knowledge check.' : 'You did not pass this knowledge check. Review the feedback above and try again.'}
          </p>
        </div>
      )}
    </div>

    ...
    <div style={{ marginTop: '2rem' }} aria-live="polite" role="status">
      ...
      {loading ? (
        ...
        <>
              ...
              return (
                <div key={key} style={{ ...requirementRowBase, border: `1px solid ${isCompleted ? 'rgba(232,184,91,0.4)' : 'rgba(243,234,216,0.12)'}` }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flex: 1 }}>
                    <span style={{ color: isCompleted ? '#e8b85b' : 'rgba(243,234,216,0.4)', fontSize: '0.9rem', fontWeight: 500 }} aria-hidden="true">
                      {isCompleted ? '\u2713' : '\u25CB'}
                    </span>
          ...
          {statusMessage && (
            <p className="font-body" role="alert" style={{ color: statusMessage.type === 'success' ? '#e8b85b' : '#e8955c', marginTop: '1rem', marginBottom: 0, fontSize: '0.88rem' }}>{statusMessage.text}</p>
          )}
          {progress.moduleCompleted && !statusMessage && (
            <p className="font-body" style={{ color: '#e8b85b', marginTop: '1rem', marginBottom: 0, fontSize: '0.9rem', fontStyle: 'italic' }}>{progressTracking.completedMessage}</p>
          )}
        </>
      ) : (

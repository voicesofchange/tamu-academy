...
const writtenAltBoxStyle = { marginTop: '1.5rem', padding: '1.4rem 1.6rem', border: '1px solid rgba(232,184,91,0.25)', borderRadius: '4px', backgroundColor: 'rgba(243,234,216,0.025)' };
...
function renderWrittenAlternative(writtenAltBoxStyle, bodyText, eyebrowStyle, section, c) {
  ...
  return (
    <div style={writtenAltBoxStyle} aria-label="Written learning alternative">
      ...
      <h4 className="font-heading" style={{ color: '#f8f0df', fontSize: '1.05rem', fontWeight: 400, margin: '0 0 0.7rem' }}>{section.heading}</h4>
      {section.paragraphs && renderParagraphs(section.paragraphs)}
      {section.numberedItems && renderNumberedItems(section.numberedItems)}
      {section.subBlocks && section.subBlocks.map((block, bi) => (
        <React.Fragment key={bi}>
...
export default function MhModule6Lesson({ course, module: mod, lesson }) {
  ...
  return (
    <ModuleLessonLayout>
      ...
      <ModuleLessonSection id="core-media" eyebrow={c.coreMediaEyebrow} heading={c.requiredVideosHeading}>
        ...
        {lesson.coreMedia.optionalExtended && lesson.coreMedia.optionalExtended.length > 0 && (
          <>
            ...
            <p className="font-body" style={{ ...bodyText, marginBottom: '1.25rem', fontSize: '0.88rem', color: 'rgba(243,234,216,0.6)' }}>{c.optionalExtendedMediaNote}</p>
      ...
      {/* 13. Applied activity */}
      <ModuleLessonSection id="amplify-lab" eyebrow={lesson.amplifyLab.eyebrow} heading={lesson.amplifyLab.title}>
        {safetyNoteBox('Content and safety note before applied activity')}
        <div aria-label="AMPLIFY safety warning" style={{ marginBottom: '1.6rem', padding: '1.4rem 1.6rem', border: '1px solid rgba(232,184,91,0.35)', borderRadius: '4px', backgroundColor: 'rgba(232,184,91,0.07)' }}>
      ...
      <ModuleLessonSection id="private-reflection" eyebrow={c.reflectionEyebrow} heading={lesson.privateReflection.heading}>
        <div aria-label="Fictional or public situation reminder" style={{ marginBottom: '1.6rem', padding: '1.4rem 1.6rem', border: '1px solid rgba(232,184,91,0.28)', borderRadius: '4px', backgroundColor: 'rgba(232,184,91,0.05)' }}>
        ...
        <p className="font-body" style={{ ...bodyText, fontStyle: 'italic', color: 'rgba(243,234,216,0.6)', fontSize: '0.85rem', marginTop: '1rem', marginBottom: 0 }}>{lesson.privateReflection.optionalNote}</p>

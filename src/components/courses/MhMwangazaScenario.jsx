...
const decisionBoxStyle = { padding: '1.4rem 1.6rem', border: '1px solid rgba(232,184,91,0.22)', borderRadius: '4px', backgroundColor: 'rgba(243,234,216,0.02)', marginBottom: '1.75rem' };
const optionBase = { display: 'flex', alignItems: 'flex-start', gap: '0.85rem', padding: '0.9rem 1.15rem', border: '1px solid rgba(232,184,91,0.22)', borderRadius: '4px', marginBottom: '0.85rem', color: 'rgba(243,234,216,0.78)', fontSize: '0.95rem', cursor: 'pointer', transition: 'border-color 0.2s ease, background-color 0.2s ease' };
const optionSelected = { ...optionBase, borderColor: 'rgba(232,184,91,0.7)', backgroundColor: 'rgba(232,184,91,0.06)' };
const feedbackBoxStyle = { marginTop: '1rem', padding: '1rem 1.25rem', border: '1px solid rgba(232,184,91,0.35)', borderRadius: '4px', backgroundColor: 'rgba(243,234,216,0.02)' };
const summaryBoxStyle = { marginTop: '2rem', padding: '1.5rem 1.75rem', border: '1px solid rgba(232,184,91,0.5)', borderRadius: '4px', backgroundColor: 'rgba(232,184,91,0.04)' };
const finalMessageBoxStyle = { marginTop: '1.5rem', padding: '1.25rem 1.5rem', border: '1px solid rgba(232,184,91,0.4)', borderRadius: '4px', backgroundColor: 'rgba(243,234,216,0.03)' };
const stepContainerStyle = { display: 'flex', gap: '0.5rem', marginBottom: '1.75rem', flexWrap: 'wrap' };
const stepButtonBase = { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '2.2rem', height: '2.2rem', borderRadius: '50%', border: '1px solid rgba(232,184,91,0.22)', backgroundColor: 'transparent', color: 'rgba(243,234,216,0.5)', fontSize: '0.85rem', fontWeight: 400, cursor: 'pointer', transition: 'border-color 0.2s ease, color 0.2s ease, background-color 0.2s ease', fontFamily: 'inherit' };
const stepButtonCurrent = { ...stepButtonBase, borderColor: '#e8b85b', color: '#e8b85b', backgroundColor: 'rgba(232,184,91,0.08)' };
const stepButtonAnswered = { ...stepButtonBase, borderColor: 'rgba(232,184,91,0.5)', color: 'rgba(232,184,91,0.85)' };
const stepButtonDisabled = { ...stepButtonBase, opacity: 0.35, cursor: 'not-allowed' };
const navButtonStyle = { display: 'inline-flex', alignItems: 'center', gap: '0.4rem', padding: '0.6rem 1.2rem', border: '1px solid rgba(232,184,91,0.4)', borderRadius: '4px', backgroundColor: 'transparent', color: 'rgba(243,234,216,0.85)', fontSize: '0.9rem', fontWeight: 400, cursor: 'pointer', transition: 'border-color 0.2s ease, background-color 0.2s ease', fontFamily: 'inherit' };
const retryButtonStyle = { ...navButtonStyle, borderColor: 'rgba(232,184,91,0.6)', color: '#E8951C', marginTop: '0.75rem' };

function optionLabel(i) { return String.fromCharCode(65 + i); }

export default function MhMwangazaScenario({ courseSlug, moduleSlug, scenario }) {
  ...
  return (
    <div>
      <p className="font-body" style={{ ...bodyText, marginBottom: '1.5rem', color: '#f8f0df', fontWeight: 400, fontSize: '1.02rem' }}>{scenario.prompt}</p>
      <p className="font-body" style={{ ...bodyText, marginBottom: '1.5rem', fontStyle: 'italic', color: 'rgba(243,234,216,0.6)', fontSize: '0.9rem' }}>{scenario.instruction}</p>
      ...
      <div style={decisionBoxStyle}>
        ...
        <h3 className="font-heading" style={{ color: '#f8f0df', fontSize: '1.15rem', fontWeight: 400, lineHeight: 1.3, margin: '0 0 0.6rem' }}>{activeDecision.heading}</h3>
        ...
        {isPending && <p className="font-body" style={{ ...bodyText, color: 'rgba(243,234,216,0.5)', fontSize: '0.85rem', margin: 0 }}>Loading feedback…</p>}
        {hasFeedback && !isPending && (
          <div role="status" aria-live="polite" style={feedbackBoxStyle}>
            ...
            <p className="font-body" style={{ ...bodyText, fontStyle: 'italic', margin: 0, color: 'rgba(243,234,216,0.7)' }}>{feedbacks[activeDecisionId]}</p>
          </div>
        )}
      ...
      {completionPending && !completed && !completionError && (
        <p className="font-body" role="status" aria-live="polite" style={{ ...bodyText, color: 'rgba(243,234,216,0.5)', fontSize: '0.88rem', marginBottom: '1rem' }}>Completing scenario…</p>
      )}
      {completionError && (
      ...
      {completed && (
        <div role="status" aria-live="polite">
          <div style={summaryBoxStyle}>
            <h3 className="font-heading" style={{ color: '#f8f0df', fontSize: '1.3rem', fontWeight: 400, margin: '0 0 1.25rem' }}>Your Design Summary</h3>
              ...
              return (
                <div key={i} style={{ marginBottom: '1.1rem' }}>
                  ...
                  {feedbacks[decision.decisionId] && (
                    <p className="font-body" style={{ ...bodyText, fontStyle: 'italic', margin: '0.35rem 0 0', color: 'rgba(243,234,216,0.6)', fontSize: '0.88rem' }}>{feedbacks[decision.decisionId]}</p>
                  )}
                </div>
              );
            })}
          ...
          <div style={finalMessageBoxStyle}>
            ...
            <p className="font-body" style={{ ...bodyText, fontStyle: 'italic', margin: '0 0 1rem', color: 'rgba(243,234,216,0.75)' }}>{scenario.strongestDesignFeature}</p>
            <span className="font-body" style={{ ...eyebrowStyle, display: 'block', marginBottom: '0.5rem' }}>Most Important Revision</span>
            <p className="font-body" style={{ ...bodyText, fontStyle: 'italic', margin: 0, color: 'rgba(243,234,216,0.75)' }}>{scenario.mostImportantRevision}</p>
          </div>
        </div>
      )}
    </div>

/**
 * Server-side-only source of truth for in-development economics course
 * module content. This file is imported ONLY by backend functions
 * (base44/functions/getModuleContent/entry.ts) and is never bundled into
 * the public client-side JavaScript. It holds the full, unreleased
 * curriculum for the pilot course — recorded lesson links, key concepts,
 * reflection questions, quiz questions and answer keys, applied activities,
 * completion requirements, closing text, and source references — that
 * must not be exposed to public visitors while the course is in
 * development.
 *
 * Public-facing course preview metadata (titles, short descriptions,
 * statuses, and completion-time estimates) lives in
 * src/lib/economics-tracks.js, which IS bundled for the browser and is
 * safe to expose publicly. The two files together form the curriculum
 * split across a server-only trust boundary: the bundle reveals only that
 * the modules are in development; the protected content is released only
 * by an authenticated, authorized admin request to the backend function.
 */

export const ECONOMICS_MODULE_CONTENT = {
 ...
 * (including reflection questions, quizzes, and answer keys) — never send
 * it back to a request that has not first passed the server-side role
 * verification in the calling backend function.
 */
export function getSensitiveModuleContent(courseSlug, moduleRoute) {
  const course = ECONOMICS_MODULE_CONTENT[courseSlug];
